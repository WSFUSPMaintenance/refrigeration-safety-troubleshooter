let devices=[],wires=[],trees={},quickReference=[],chains=[],currentTree=null,currentNodeId=null;
...
async function loadData(){
  try{
    ...
    setupTabs();renderSymptoms();renderQuickReference();renderChains();setupSearch();
  }catch(err){
...
function renderNode(){
  ...
  document.getElementById('stepCard').innerHTML=`<div class="card"><h3>${node.device}</h3><p class="meta"><strong>Question:</strong> ${node.question}</p><p class="meta"><strong>Expected:</strong> ${node.expected}</p><p class="meta"><strong>Drawing:</strong> ${node.drawing||''} ${node.line?'- Line '+node.line:''}</p><div class="choiceRow"><button onclick="answer('yes')">YES / VOLTAGE PRESENT / PASS</button><button onclick="answer('no')">NO / VOLTAGE ABSENT / FAIL</button></div></div>`;
}
...
function normalizeSearch(v){
  return String(v||'').toLowerCase().replace(/wire\s*/g,'').replace(/device\s*/g,'').replace(/relay\s*/g,'').replace(/sensor\s*/g,'').replace(/[^a-z0-9]/g,'');
}

function searchableText(o){
  return [JSON.stringify(o),o.id,o.wire,o.description,o.type,o.drawing,o.line,o.location,o.expectedVoltage,o.normalState,o.failureEffect,o.zone,o.branch].filter(Boolean).join(' ');
}
...
function chainForId(id){
  ...
  return chains.find(c => c.nodes.map(n=>String(n).toLowerCase()).includes(clean) || String(c.id).toLowerCase()===clean);
}
...
function search(){
  ...
  if(deviceResults.length===0&&wireResults.length===0&&chainResults.length===0){
    el.innerHTML=`<div class="card warn"><h3>No match found</h3><p class="meta">Search entered: <strong>${raw}</strong></p><p class="meta">Try: <strong>CR407</strong>, <strong>Sensor 6</strong>, <strong>EB-MR-4</strong>, <strong>617</strong>, <strong>Debone</strong>, <strong>HTR1</strong>, or <strong>NH3</strong>.</p></div>`;
    return;
  }

  let html=`<div class="card"><strong>${deviceResults.length}</strong> device match(es), <strong>${wireResults.length}</strong> wire match(es), <strong>${chainResults.length}</strong> chain match(es) for "${raw}".</div>`;

  if(chainResults.length){
    html+='<h3>Cause Chains</h3>'+chainResults.map(c=>chainCard(c)).join('');
  }

  if(deviceResults.length){
    html+='<h3>Devices</h3>'+deviceResults.map(d=>{
      const c=chainForId(d.id);
      const chainHtml=c?`<p class="meta"><strong>Cause Chain:</strong> ${c.nodes.join(' → ')}</p>`:'';
  ...
  if(wireResults.length){
    html+='<h3>Wires</h3>'+wireResults.map(w=>{
